...
const AllBlogs = () => {
    ...
    const { blogs, navigate, backendUrl, user, filterUserSavedBlogs, fetchSavedBlogs, isAuth, setShowLogin, loading } = useContext(AppContext);
                        ...
                        blogs.map((blog) => {
                            return (
                                <div key={blog.id} className='cursor-pointer text-black rounded-lg'>
                                    <div className='rounded-lg relative group w-full h-[250px]'>
                                        <motion.img
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 1 }}
                                            transition={{ duration: 0.3 }}
                                            src={blog.posterUrl} alt={blog.title} className='rounded-ss-lg rounded-se-lg object-cover h-full w-full' ></motion.img >
